...
This report uses the You Only Look Once (YOLO) v8 model. YOLO models are single stage real-time object detectors which are deep learning models that are able to find and identify objects in a single pass through a Convolutional Neural Network (CNN). This CNN is also known as the backbone of the YOLO model. This backbone will be further elaborated on in the next section. Unlike two-stage object detectors, YOLO models immediately predict bounding boxes and class probabilities from the input image in a single forward pass. This makes YOLO extremely fast at object detection. Furthermore, YOLOv8 differs from its earlier versions by enabling anchor-free detection, whereby object centres' coordinates and bounding boxes dimensions are directly predicted instead of being predefined. YOLO models are widely used for modeling object detection as they are faster, trainable on a single GPU, and can be used for real-time detection. YOLOv8 achieves this with very high accuracy and with optimal well-balanced precision-recall tradeoffs.

### How YOLOv8 Works
In a YOLO model, the input image is passed through the CNN which is also known as the model's backbone. This step extracts important features that YOLO will use to find and classify objects. Such features may include: textures, shapes, and object edges. These features are merged by the "neck" using the Path Aggregation Network (PAN) instead of the Feature Pyramid Network (FPN) of earlier YOLO models. This combines high-level semantic features with low-level spatial information, which then enables objects of varying sizes to be detected with high accuracy. In this context, YOLOv8 is used to detect lesions. As the merged features are passed to the anchor-free detection head of YOLOv8, bounding boxes, the confidence of the model regarding the existence of said object, and class probabilities are predicted. Unlike earlier versions, YOLOv8 is anchor-free and predicts object centers straightaway. The predictions derived are subsequently used to obtain the finalised object detections. Each of YOLO's detection grid cell outputs a few of such predictions, which includes their respective confidence scores. Finally, YOLO uses Non-Maximum Suppression (NMS) to filter overlapping boxes, resulting in one bounding box with the highest confidence score.
...
Firstly, each raw image and its corresponding mask are read in RGB and grayscale formats respectively. Subsequently, the regions and contours as to where the lesions are located are extracted from these masks. For each lesion detection, the smallest possible rectangular region that encloses the lesion was computed. These regions are then converted into the lesions' bounding boxes. Then, these coordinates were normalised to ensure that there does not exist a variance in scale for images of varied resolutions. From these steps, we derive the YOLO-format labels (as of required of the algorithm) for each image which are saved as .txt files. An additional step of organising the file structure into images and labels separately was implemented. This was done to prevent confusion when training, evaluating and testing the model in later parts.
...
![Precision-Confidence Plot](pics_for_readme/BoxP_curve.png)
...
![Example Input Image 1](pics_for_readme/ISIC_0015485_copy.jpg)
![Corresponding Output Image 1](pics_for_readme/ISIC_0015485.jpg)
...
![Example Input Image 2](pics_for_readme/ISIC_0015963_copy.jpg)
![Corresponding Output Image 2](pics_for_readme/ISIC_0015963.jpg)
